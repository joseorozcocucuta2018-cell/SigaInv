...
beforeEach(function () {
    ...
    Numeracion::create(['tipo_documento' => 'venta', 'resolucion_numero' => 'R-MIX', 'prefijo' => 'POS', 'consecutivo_desde' => 1, 'consecutivo_hasta' => 9999, 'consecutivo_actual' => 0, 'anno' => now()->year, 'estado' => NumeracionEstado::ACTIVO]);

    $this->formaEfectivo = FormaPago::create(['nombre' => 'Efectivo', 'activo' => true, 'requiere_banco' => false]);
    $this->formaTransferencia = FormaPago::create(['nombre' => 'Transferencia', 'activo' => true, 'requiere_banco' => true]);

    TurnoService::abrir($this->user, ['caja_id' => $caja->id, 'bodega_id' => $bodega->id, 'saldo_inicial' => 0]);
});

it('acepta pago mixto efectivo + transferencia cubriendo exacto', function () {
    $response = $this->withToken($this->token)->postJson('/pos/api/ventas', [
        ...
        'pagos' => [
            ...
            ['forma_pago_id' => $this->formaTransferencia->id, 'monto' => 1380],
        ],
    ]);

    $response->assertStatus(201);
    $this->assertEquals(2, PagoCliente::where('cliente_id', $this->cliente->id)->count());
});

it('rechaza pago mixto con no-efectivo excedente', function () {
    $response = $this->withToken($this->token)->postJson('/pos/api/ventas', [
        'cliente_id' => $this->cliente->id,
        'items' => [['producto_id' => $this->producto->id, 'cantidad' => 2]],
        'pagos' => [
            ['forma_pago_id' => $this->formaEfectivo->id, 'monto' => 500],
            ['forma_pago_id' => $this->formaTransferencia->id, 'monto' => 2000],
        ],
    ]);
