...
beforeEach(function () {
    ...
    Numeracion::create(['tipo_documento' => 'venta', 'resolucion_numero' => 'R-002', 'prefijo' => 'POS', 'consecutivo_desde' => 1, 'consecutivo_hasta' => 9999, 'consecutivo_actual' => 0, 'anno' => now()->year, 'estado' => NumeracionEstado::ACTIVO]);
...
it('crea venta con pago menor al total registra saldo pendiente', function () {
    $response = $this->withToken($this->token)->postJson('/pos/api/ventas', [
        'cliente_id' => $this->cliente->id,
        'items' => [['producto_id' => $this->producto->id, 'cantidad' => 2]],
        'pagos' => [['forma_pago_id' => $this->formaPago->id, 'monto' => 1000]],
    ]);
