...
beforeEach(function () {
    ...
    StockBodega::create(['producto_id' => $this->producto->id, 'bodega_id' => $bodega->id, 'cantidad' => 100]);
    Numeracion::create(['tipo_documento' => 'venta', 'resolucion_numero' => 'R-001', 'prefijo' => 'POS', 'consecutivo_desde' => 1, 'consecutivo_hasta' => 9999, 'consecutivo_actual' => 0, 'anno' => now()->year, 'estado' => NumeracionEstado::ACTIVO]);
...
it('crea venta contado vía API POS', function () {
    $response = $this->withToken($this->token)->postJson('/pos/api/ventas', [
        'cliente_id' => $this->cliente->id,
        'items' => [['producto_id' => $this->producto->id, 'cantidad' => 2]],
        'pagos' => [['forma_pago_id' => $this->formaPago->id, 'monto' => 2380]],
    ]);

    $response->assertStatus(201);
    $response->assertJsonStructure(['id', 'numero', 'total', 'data']);

    $this->assertDatabaseHas('ventas', ['id' => $response->json('id'), 'estado' => VentaEstado::PAGADA->value]);
    $this->assertDatabaseHas('pago_clientes', ['cliente_id' => $this->cliente->id]);
});

it('crea venta contado con stock correcto', function () {
    $this->withToken($this->token)->postJson('/pos/api/ventas', [
        'cliente_id' => $this->cliente->id,
        'items' => [['producto_id' => $this->producto->id, 'cantidad' => 5]],
        'pagos' => [['forma_pago_id' => $this->formaPago->id, 'monto' => 5950]],
    ]);
