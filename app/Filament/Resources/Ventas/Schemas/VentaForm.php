                ...
                Section::make('Datos Generales')
                    ...
                    ->schema([
                        ...
                        Select::make('cotizacion_id')
                            ->label('Cotización')
                            ->options(function () {
                                return Cotizacion::where(function ($query) {
                                    $query->where('estado', CotizacionEstado::PENDIENTE->value)
                                        ->orWhere('estado', CotizacionEstado::ACEPTADA->value);
                                })
                            ...
                            ->afterStateUpdated(function (Set $set, Get $get, ?string $state) {
                                ...
                                // Validar que la cotización sea válida
                                try {
                        ...
                        Select::make('remision_id')
                            ->label('Remisión')
                            ->options(
                                Remision::where('estado', RemisionEstado::CONFIRMADA)
                                    ->pluck('numero', 'id')
                            )
                            ->searchable()
                            ->placeholder('Ninguna'),
                ...
                Section::make('Detalle de Productos')
                    ...
                    ->schema([
                        Repeater::make('detalles')
                            ->relationship()
                            ->columns(7)
                            ->minItems(1, 'Debe agregar al menos un detalle a la venta')
                            ->schema([
                                ...
                                TextInput::make('serial')
                                    ...
                                    ->visible(function (Get $get): bool {
                                        ...
                                        if (! $producto || ! $producto->exige_serial) {
                                            return false;
                                        }
                                        $empresa = Empresa::first();

                                        return $empresa && $empresa->usa_seriales;
                                    })
                                    ->rule(function (Get $get) {
                                        return function (string $attribute, $value, \Closure $fail) use ($get) {
                                            ...
                                            $empresa = Empresa::first();
                                            if (! $empresa || ! $empresa->usa_seriales) {
                                                return;
                                            }
                                    ...
                                    ->rules([
                                        fn (Get $get): \Closure => function (string $attribute, $value, \Closure $fail) use ($get) {
                                            ...
                                            if ($producto && $producto->exige_serial && $empresa && $empresa->usa_seriales) {
                                                if ((float) $value !== 1.0) {
                                                    $fail('Los productos controlados por serial deben venderse con cantidad 1 por línea.');
